  ...
  const handleSearch = async (ordNum: string, em: string) => {
    ...
    try {
      ...
      if (response.ok && data.success && data.order) {
        ...
        const local = getLocalOrder(ordNum);
        if (local) {
          if (em && local.email && local.email.toLowerCase() !== em.trim().toLowerCase()) {
            showToast('Email Mismatch', 'Email does not match records for this order reference.', 'error');
          } else {
            setOrder(local);
          }
        } else {
